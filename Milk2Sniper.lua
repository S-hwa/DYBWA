...
local SETTINGS_FILE = "PetScannerSettings.json"
local WEBHOOK_URL = "YOUR_DISCORD_WEBHOOK_HERE" -- Placeholder for your safety
...
-- =============================================================================
local function forceLoadingComplete()
    local flags = {
        PersistentLoaded = true, ControllersStarted = true, DataLoaded = true,
        LoadingScreenActive = false, LoadingScreenDone = true,
        GardenLoadingTotal = 1, GardenLoadingProgress = 1
    }
    for flag, value in pairs(flags) do
        player:SetAttribute(flag, value)
    end
end

local function killLoadingScreen()
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj.Name == "LoadingScreenMenu" or obj:FindFirstChild("LoadingGui", true) then
            obj:Destroy()
        end
    end
    for _, gui in ipairs(playerGui:GetChildren()) do
        if gui:FindFirstChild("LoadingGui", true) or gui.Name:find("Loading") then
            gui:Destroy()
        end
    end
end

local function restoreCameraAndCharacter()
...
print("🚀 Loading Skipper Activated")
forceLoadingComplete()
killLoadingScreen()
restoreCameraAndCharacter()

task.delay(0.5, function()
    UserInputService.InputBegan:Fire(UserInputService, {KeyCode = Enum.KeyCode.Space}, false)
end)

for _, script in ipairs(player.PlayerScripts:GetDescendants()) do
    if script:IsA("LocalScript") and (script.Name:find("Loading") or script.Source:find("LoadingScreen")) then
        script.Disabled = true
        task.delay(0.1, function() script:Destroy() end)
    end
end
    ...
    pcall(function()
        if isfile and isfile(SETTINGS_FILE) then
            ...
            if type(data) == "table" then
                if _G.PetScannerAutoScan == nil and data.autoScan ~= nil then
                    _G.PetScannerAutoScan = data.autoScan
                end
                if _G.PetScannerAutoHop == nil and data.autoHop ~= nil then
                    autoHop = data.autoHop
                end
            end
        end
    end)
    -- Overrides for active Server Hopping priority
...
local function isTargeted(pet)
    return checkedPets[pet.name] == true
end
...
local function autoBuy(pet)
    ...
    if fireproximityprompt then
        ...
        warn("Your executor does not support fireproximityprompt!")
        return false
    end
end
...
local function hopServer()
    ...
    pcall(function()
        local module = loadstring(game:HttpGet("https://raw.githubusercontent.com/AwesomeDudePerfect/psx-gem-farm/refs/heads/main/tp.lua"))()
    end)
    task.wait(5)
    hopCooldown = false
end
...
createInstance("Frame", {Size = UDim2.new(0, 10, 0, 10), Position = UDim2.new(0, 14, 0.5, -5), BackgroundColor3 = Color3.fromRGB(180, 120, 255), BorderSizePixel = 0}, header):CreateInstance("UICorner", {CornerRadius = UDim.new(1, 0)})
createInstance("TextLabel", {Size = UDim2.new(1, -130, 1, 0), Position = UDim2.new(0, 30, 0, 0), BackgroundTransparency = 1, Text = "PET SCANNER", Font = Enum.Font.GothamBlack, TextSize = 13, TextTextColor3 = Color3.fromRGB(180, 120, 255), TextXAlignment = Enum.TextXAlignment.Left}, header)
...
createInstance("TextLabel", {Size = UDim2.new(1, 0, 0, 18), BackgroundTransparency = 1, Text = "TARGET PETS (regardless of size)", Font = Enum.Font.GothamBlack, TextSize = 9, TextTextColor3 = Color3.fromRGB(180, 120, 255), TextXAlignment = Enum.TextXAlignment.Left}, leftCol)
...
createInstance("TextLabel", {Size = UDim2.new(1, 0, 0, 18), BackgroundTransparency = 1, Text = "CURRENT PETS IN SERVER", Font = Enum.Font.GothamBlack, TextSize = 9, TextTextColor3 = Color3.fromRGB(180, 120, 255), TextXAlignment = Enum.TextXAlignment.Left}, rightCol)
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    saveSettings() -- Instantly saves to settings file
end)
...
print("Pet Scanner settings persistence loaded perfectly!")